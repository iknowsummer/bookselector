...
def read_shelves(current_user: CurrentUser, db: Session = Depends(get_db)):
    ...
    logger.info(
        "GET /shelves - Retrieved %s shelves for user_id=%s",
        len(shelves),
        current_user.id,
    )
    return shelves
...
def read_shelf(shelf_id: int, current_user: CurrentUser, db: Session = Depends(get_db)):
    """棚詳細を取得 (user-scoped)"""
    logger.info(
        "GET /shelves/%s - Fetching shelf detail for user_id=%s",
        shelf_id,
        current_user.id,
    )
    shelf = (
    ...
    if not shelf:
        logger.warning(
            "GET /shelves/%s - Shelf not found for user_id=%s",
            shelf_id,
            current_user.id,
        )
...
def create_shelf(
    ...
    """棚を新規作成 (user-scoped)"""
    logger.info(
        "POST /shelves - Creating shelf: name='%s' for user_id=%s",
        shelf.name,
        current_user.id,
    )
    try:
        ...
        db.refresh(db_shelf)
        logger.info(
            "POST /shelves - Created shelf id=%s for user_id=%s",
            db_shelf.id,
            current_user.id,
        )
        return db_shelf
...
def update_shelf(
    ...
    """棚情報を更新 (user-scoped)"""
    logger.info(
        "PUT /shelves/%s - Updating shelf for user_id=%s", shelf_id, current_user.id
    )
    try:
        db_shelf = (
            db.query(Shelf)
        ...
        if not db_shelf:
            logger.warning(
                "PUT /shelves/%s - Shelf not found for user_id=%s",
                shelf_id,
                current_user.id,
            )
            raise HTTPException(status_code=404, detail="Shelf not found")
        ...
        db.refresh(db_shelf)
        logger.info(
            "PUT /shelves/%s - Updated shelf for user_id=%s", shelf_id, current_user.id
        )
        return db_shelf
        ...
        db.rollback()
        logger.error(
            "PUT /shelves/%s - Database error: %s", shelf_id, exc, exc_info=True
        )
...
def delete_shelf(
    ...
    """棚を削除 (user-scoped)"""
    logger.info(
        "DELETE /shelves/%s - Deleting shelf for user_id=%s", shelf_id, current_user.id
    )
    try:
        shelf = (
            db.query(Shelf)
        ...
        if not shelf:
            logger.warning(
                "DELETE /shelves/%s - Shelf not found for user_id=%s",
                shelf_id,
                current_user.id,
            )
            raise HTTPException(status_code=404, detail="Shelf not found")
        ...
        db.commit()
        logger.info(
            "DELETE /shelves/%s - Deleted shelf for user_id=%s",
            shelf_id,
            current_user.id,
        )
        ...
        db.rollback()
        logger.error(
            "DELETE /shelves/%s - Database error: %s", shelf_id, exc, exc_info=True
        )
